           ...
           <Button className="mt-6" onClick={() => navigate("/products")}>Shop Now</Button>
            ...
            <div className="grid gap-8 lg:grid-cols-3">
             <div className="lg:col-span-2 space-y-6">
                {
                    cart.map((e)=>(
                        <div key={e._id} className="flex flex-col sm:flex-row items-center
                         sm:items-stretch space-y-4 sm:space-y-0 sm:space-x-4 shadow-md rounded-lg p-4 border border-gray-400">
                          <img src={e.product.images[0].url} alt={e.product.title}
                          className="w-full sm:w-20 sm:h-20 object-contain rounded-md cursor-pointer"
                           onClick={()=>navigate(`/product/${e.product._id}`)} 
                         />
                         <div className="flex-1 text-center sm:text-left">
                            <h2 className="text-lg font-medium">{e.product.title}</h2>
                            <p>Price: EGP {e.product.price}</p>
                         </div>
             ...
             <div className="p-6 shadow-lg rounded-lg border border-gray-400">
                ...
                <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                        ...
                        <span>Total Price - EGP{subTotal}</span>
                    </div>
                </div>
