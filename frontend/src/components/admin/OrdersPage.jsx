...
function OrdersPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Input className="w-full md:w-1/2" value={search} placeholder="search by email or order id"
            ...
            <TableBody>
                {
                    filteredOrders.map((order)=>(
                        <TableRow key={order._id}>
                            ...
                            <TableCell> <span className={`px-2 py-1 rounded text-white ${order.status === "pending" ? "bg-yellow-500" : order.status === "Shipped" ? "bg-blue-500" : "bg-green-500"}`}>
                                    {order.status} </span> 
                            </TableCell>
